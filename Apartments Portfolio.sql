...
-- CTE presents number of apartments, price per square meter, average price and various facilities 

WITH CTE_CityStatistics AS (
    SELECT
        b.city
        ,COUNT(*) AS TotalApartments
        ,ROUND(AVG(p.price / b.squareMeters),0) AS AvgPricePerSqM
        ,ROUND(AVG(p.price),0) AS AvgPrice
        ,SUM(CASE WHEN f.hasParkingSpace = 'yes' THEN 1 ELSE 0 END) AS ParkingCount
        ,SUM(CASE WHEN f.hasBalcony = 'yes' THEN 1 ELSE 0 END) AS BalconyCount
        ,SUM(CASE WHEN f.hasElevator = 'yes' THEN 1 ELSE 0 END) AS ElevatorCount
        ,SUM(CASE WHEN f.hasSecurity = 'yes' THEN 1 ELSE 0 END) AS SecurityCount
        ,SUM(CASE WHEN f.hasStorageRoom = 'yes' THEN 1 ELSE 0 END) AS StorageCount
    FROM Portfoliov2.dbo.basic$ b
			JOIN Portfoliov2.dbo.price$ p 
			ON b.id = p.id
				JOIN Portfoliov2.dbo.facilities$ f 
